...
execute as @a if score @s glass matches 1 run tellraw @a [{"type":"translatable","translate":"things.stone.disappearance.message.enable.a","fallback":"[数据包消息]"},{"text":" "},{"type":"selector","selector":"@s"},{"text":" "},{"type":"translatable","translate":"things.stone.disappearance.message.enable.b","fallback":"开启了填充玻璃"}]
execute as @a if score @s glass matches 1 run scoreboard players set @a glass 2
execute as @a if score @s glass matches 3 run tellraw @a [{"type":"translatable","translate":"things.stone.disappearance.message.disable.a","fallback":"[数据包消息]"},{"text":" "},{"type":"selector","selector":"@s"},{"text":" "},{"type":"translatable","translate":"things.stone.disappearance.message.disable.b","fallback":"关闭了填充玻璃"}]
